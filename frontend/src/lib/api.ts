/** Client API pour lebeSsni — appels same-origin (plus de backend Python séparé) */
...
export async function detectMode(
  ...
  const res = await fetch("/api/tryon/detect-mode", { method: "POST", body: form });
...
export async function generateTryOn(
  ...
  form.append("request", JSON.stringify({
    mode, type_produit: productType, zone_corps: "haut du corps",
    style_rendu, orientation,
  }));
...
export async function getDashboardProduits(): Promise<DashboardProduit[]> {
  const res = await fetch("/api/dashboard/produits", { headers: { ...authHeaders() } });
...
export async function createDashboardProduit(data: {
  nom: string; description?: string; product_type: string; image_url: string;
}): Promise<DashboardProduit> {
  const res = await fetch("/api/dashboard/produits", {
...
export async function deleteDashboardProduit(id: number): Promise<void> {
  const res = await fetch(`/api/dashboard/produits/${id}`, {
...
export interface DashboardCredits {
  plan: string; credits_total: number; credits_used: number; credits_remaining: number;
...
export async function getDashboardCredits(): Promise<DashboardCredits> {
  const res = await fetch("/api/dashboard/credits", { headers: { ...authHeaders() } });
  if (res.status === 401) { if (typeof window !== "undefined") window.location.href = "/login"; throw new Error("Non authentifié"); }
...
export interface DashboardStats {
  visiteurs: number; generations: number; clics_achat: number;
